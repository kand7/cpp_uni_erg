...
private:
	...
	string subId;
	string subName;
	unsigned int subHours;
	string subSemester;

public:
	// ΜΕΘΟΔΟΙ
	Subject(string, string, unsigned int, string); // ΜΟΝΑΔΙΑΙΟΣ ΚΑΤΑΣΚΕΥΑΣΤΗΣ ΜΕ ΟΛΕΣ ΤΙΣ ΠΑΡΑΜΕΤΡΟΥΣ
	void setSubId(string);
	void setSubName(string);
	void setSubHours(unsigned int);
	void setSubSemester(string);
	string getSubId();
	string getSubName();
	unsigned int getSubHours();
	string getSubSemester();
};
// ΥΛΟΠΟΙΗΣΗ ΜΟΝΑΔΙΚΟΥ ΚΑΤΑΣΚΕΥΑΣΗ
Subject::Subject(string subId, string subName, unsigned int subHours, string subSemester) : subId{subId}, subName{subName}, subHours{subHours}
{
}

// ΤΩΡΑ ΑΚΟΛΟΥΘΟΥΝ ΟΙ GETTERS ΚΑΙ SETTERS
void Subject::setSubId(string subId)
{
	this->subId = subId;
}
...
void Subject::setSubHours(unsigned int subHours)
{
	this->subHours = subHours;
}
void Subject::setSubSemester(string subSemester)
{
	cout << "subSemester : " << subSemester << endl;
	while (subSemester != "Χειμερινό" and subSemester != "Εαρινό")
	{
		cout << "\nΈγκυρο εξάμηνο θεωρείται μόνο το Χειμερινό ή το Εαρινό.\nΔώσε σωστό εξάμηνο : ";
		cin >> subSemester;

		// subSemester = tolower(subSemester[0]);

		cout << subSemester << endl;
	}
